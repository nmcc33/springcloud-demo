...
#JAVA_OPTS="-server -Xms2G -Xmx2G -Xss512k -XX:MetaspaceSize=60m -XX:MaxMetaspaceSize=256m"
...
start(){
 ...
 if [ "$PROFILE_ACTIVE" = "dev" ] || [ "$PROFILE_ACTIVE" = "test" ] ; then
	JAVA_OPTS="-server -Xms1G -Xmx1G -Xss512k -XX:MetaspaceSize=256m -XX:MaxMetaspaceSize=256m"
 fi
 if [ -z "$JAVA_OPTS" ] ;then
 	JAVA_OPTS="-server -Xms1G -Xmx1G -Xss512k -XX:MetaspaceSize=60m -XX:MaxMetaspaceSize=128m"
 fi
